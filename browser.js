...
// // =========================================================================
// // 🔄 MAIN LOOP: Refresh par sirf Browser band hoga, FFmpeg chalta rahega!
// // =========================================================================
// async function mainLoop() {
//     // FFmpeg ko sab se pehle start kar do, taake OK.ru par stream shuru ho jaye (White Screen ke sath)
//     startBroadcast();
...
//         } catch (error) {
//             console.error(`\n[!] WATCHDOG ALERT: ${error.message}`);
//             console.log('[*] 🔄 Restarting Browser in 3 seconds. (OK.ru Stream is STILL LIVE with White Screen)...');
//             await cleanupForRefresh(); // Sirf browser band karega, FFmpeg nahi!
...
//     // 🛡️ NAYA MAGIC: Hiding the Website during Load!
//     await page.evaluateOnNewDocument(() => {
//         document.addEventListener('DOMContentLoaded', () => {
//             const blocker = document.createElement('div');
//             blocker.id = 'okru-white-loading-screen';
...
//             blocker.style.fontSize = '35px';
//             blocker.style.fontWeight = 'bold';
//             blocker.style.color = '#333';
//             blocker.innerHTML = 'Stream is connecting... Please wait.';
...
//     console.log(`🎥 [*] 20-second Debug Recording Started...`);

//     setTimeout(async () => {
//         try {
//             await recorder.stop();
//             console.log('🛑 [*] Uploading recording to GitHub Releases...');
//             execSync(`gh release create visual-debug-${Date.now()} ${fileName} --title "Stream Refresh Debug"`, { stdio: 'inherit' });
//         } catch (err) { }
//     }, 20000);
...
//     console.log('[*] Waiting for Cloudflare & Load...');
...
//             if (isRealLiveStream) targetFrame = frame;
...
//     // 🔊 AUDIO UNLOCKER
//     console.log('[*] Applying Center Click & Unmute...');
//     try {
...
//         if (video) { video.muted = false; video.volume = 1.0; await video.play().catch(e => {}); }
//     });

//     // 📺 FULLSCREEN HACK
//     console.log('[*] Executing Fullscreen Hack...');
//     await targetFrame.evaluate(async () => {
//         const vid = document.querySelector('video');
//         if (!vid) return;
//         try {
...
//     // 🔓 REVEAL THE STREAM
//     console.log('\n[🎉] SUCCESS! Video is Fullscreen. Removing White Loading Screen from OK.ru...');
//     await page.evaluate(() => {
//         const blocker = document.getElementById('okru-white-loading-screen');
//         if (blocker) blocker.remove();
//     });

//     // =========================================================================
//     // 🧠 THE ENHANCED SMART WATCHDOG (Pause-Healer Logic Added!)
//     // =========================================================================
//     console.log('\n[*] Smart Engine Connected! Monitoring Video Pause/Crash 24/7...');
    
//     while (true) {
//         if (!browser || !browser.isConnected()) throw new Error("Browser closed.");

//         // 🚀 Watchdog ab 4 states return karega
//         const healthStatus = await targetFrame.evaluate(async () => {
//             const v = document.querySelector('video');
            
//             if (!v) return 'NO_VIDEO'; // Video element hi gayab
//             if (v.ended) return 'ENDED'; // Match/Stream khtam
//             if (v.clientWidth <= (window.innerWidth * 0.5)) return 'AD_OVERLAY'; // Ad ne video choti kar di
            
//             if (v.paused) {
//                 // 🛠️ PAUSE HEALER: Agar video pause ho jaye, usko zabardasti chalao!
//                 try {
//                     await v.play();
//                     return 'PAUSED_RECOVERED'; // Recover ho gayi
//                 } catch (err) {
//                     return 'PAUSED_STUCK'; // Atak gayi, refresh zaroori hai
//                 }
//             }
...
//         // Result Analysis
//         if (healthStatus === 'PAUSED_RECOVERED') {
//             console.log('[!] Watchdog Notice: Video paused due to buffering, but auto-played successfully!');
//         } 
//         else if (healthStatus !== 'HEALTHY') {
//             console.log('\n=================================================================');
//             console.log(`❌ ❌ ❌ WATCHDOG ERROR: ${healthStatus} ❌ ❌ ❌`);
//             console.log('🛑 TRIGGERING BROWSER REFRESH (FFmpeg will stay live)...');
//             console.log('=================================================================\n');
//             throw new Error(`Stream interrupted. Reason: ${healthStatus}`);
//         }

//         await new Promise(r => setTimeout(r, 3000)); // Har 3 second check
//     }
// }

// // 🧹 CLEANUP FUNCTIONS
// async function cleanupForRefresh() {
//     if (browser) {
...
//     console.log('[*] Stealth Mode: Unmuting video and hiding player UI...');
//     await targetFrame.evaluate(async () => {
//         // Hamesha ke liye player ke controls aur ads ko CSS se ghayab karo
//         const style = document.createElement('style');
//         style.innerHTML = `
...
//         // Raw element se aawaz kholo (No click!)
//         const video = document.querySelector('video');
//         if (video) {
//             video.muted = false; 
//             video.volume = 1.0; 
...
// =============== 100% teek hai ok.ru mei bas eek issue bar bar araha hai k jab stream off hu jayee crichd website mei too yeh full webiste ko ok.ru me show karta hai ================
...
// ===================== done, bas yeh stream off honey k waja see ok.ru ko kill karney k baad yeh pher  se start nahey kar pa raha ahi yeh belwo code, opper code mei video add key hai and refresh the page functionality add karty hai =========================
...
//         const box = await iframeEl.boundingBox();
//         if (box) {
//             // Video ke bilkul center par ek physical click
//             await page.mouse.click(box.x + (box.width / 2), box.y + (box.height / 2), { delay: 100 });
//             console.log('[+] Real physical click executed on the center of the video player.');
//         }
//         await new Promise(r => setTimeout(r, 2000));
...
//             for (const frame of page.frames()) {
//                 try {
//                     const isRealVid = await frame.evaluate(() => {
//                         const vid = document.querySelector('video');
//                         return vid && vid.clientWidth >= 300 && vid.clientHeight >= 200;
//                     });
//                     if (isRealVid) { targetFrame = frame; break; }
//                 } catch(ex){}
//             }
//         }
...
// # 0 ============= Done , bass aab multi stream add karney hai ok.ru ======================= 
...
// // 🚀 NAYA: Ab URL hardcoded nahi hai, balke GitHub Actions input se aayega
// const TARGET_URL = process.env.TARGET_URL || 'https://dadocric.st/player.php?id=starsp3&v=m';
// const RTMP_SERVER = 'rtmp://vsu.okcdn.ru/input/';
...
//     // YML file se quality input catch kar rahe hain
//     const streamQuality = process.env.STREAM_QUALITY || '110KBps (Balanced 480p)';
...
//     // =========================================================================
//     // 🧠 THE SMART SCANNER (Finds the REAL stream, ignores tiny Video Ads)
//     // =========================================================================
//     let targetFrame = null;
//     console.log('[*] Scanning iframes for the REAL Live Stream Video...');
//     for (const frame of page.frames()) {
...
//     // =========================================================================
//     // 🔊 THE PHYSICAL CLICK & UNMUTE LOGIC (New Feature for fixing Audio)
//     // =========================================================================
//     console.log('[*] Executing Physical Click to bypass Browser Audio Restrictions...');
//     try {
//         // 1. Aapki picture se cross button ka exact selector
//         const crossSelector = 'img[src*="0P0Udsa.png"]';
//         const crossBtn = await targetFrame.$(crossSelector);

//         if (crossBtn) {
//             console.log('[+] Target Lock! Cross (X) overlay mil gaya. Clicking it...');
//             await crossBtn.click();
//         } else {
//             console.log('[-] Cross nahi mila, falling back to Video Center Click...');
//             // 2. Agar cross na mile, toh video ke beech mein click karega
//             const iframeEl = await targetFrame.frameElement();
//             const box = await iframeEl.boundingBox();
//             if (box) {
//                 await page.mouse.click(box.x + (box.width / 2), box.y + (box.height / 2));
//                 console.log('[+] Clicked center of the video player.');
//             }
//         }
        
//         // Wait for the popup/overlay to disappear and popup blocker to kill any new tabs
//         await new Promise(r => setTimeout(r, 3000));
//     } catch (e) {
//         console.log('[!] Overlay click mechanism failed: ', e.message);
//     }

//     console.log('[*] Forcing JS Unmute & Volume Max as backup...');
//     await targetFrame.evaluate(async () => {
//         const video = document.querySelector('video');
//         if (!video) return false;
        
//         video.muted = false; 
//         video.volume = 1.0;  // Volume full
...
//         // YML Input ke mutabiq dynamic settings
//         if (streamQuality.includes('40KBps')) {
//             console.log('\n[*] 🚀 FFmpeg Mode: ULTRA-LOW BANDWIDTH (360p @ 20FPS, 200k Video, 32k Audio)...');
//             ffmpegArgs = [
//                 '-y', '-use_wallclock_as_timestamps', '1', '-thread_queue_size', '1024',
...
//                 // 1. Agar video tag nahi hai toh aage barho
//                 if (!vid) return false;
                
//                 // 2. SIZE CHECK: Agar video choti hai (Ad hai), toh ignore karo
//                 if (vid.clientWidth < 300 || vid.clientHeight < 200) return false;

//                 // Yeh asli video lag rahi hai
//                 return true; 
//             });
...
//                 targetFrame = frame;
//                 console.log(`[+] Smart Scanner selected Real Video in frame: ${frame.url() || 'unknown'}`);
//                 break; // Asli video mil gayi, mazeed dhoondna band karo
//             }
//         } catch (e) { }
//     }
...
//     console.log('[*] Executing Audio Unmute, Volume Max, and Wait Logic...');
//     await targetFrame.evaluate(async () => {
//         const video = document.querySelector('video');
//         if (!video) return false;
        
//         video.muted = false; // Video unmute kar di
//         video.volume = 1.0;  // 🚀 NAYA: Volume ko 100% (Full) kar diya
...
//             // Watchdog ko bhi smart bana diya, taake reload hone par asli video hi pakray
//             for (const frame of page.frames()) {
//                 try {
//                     const isRealVid = await frame.evaluate(() => {
//                         const vid = document.querySelector('video');
...
// ================ great bas aab oopper waley code mei eek new manually input add karna hai taakey mei manually url dee sko ============================





// Agar kisi din dadocric walo ne page ke top par koi aisi ad laga di jo <video> tag use karti ho (jaise kisi game ka promo video), toh aapka bot ghalti se us ad ko pakar lega aur 24/7 us ad ko OK.ru par stream karta rahega!

// 🛡️ Isko 100% Bulletproof Kaise Banayen? (The Smart Upgrade)
// Kyunke aap ek production-grade system bana rahe hain, aapko document.querySelector('video') ko thora "Smart" banana hoga taake woh Asli Stream aur Ad Video ke beech farq kar sake.

// Asli video ki 2 pehchan hoti hain:

// Uski screen par Width aur Height bari hoti hai (Ads chote hote hain).

// Uska link aam tor par blob: se shuru hota hai (jaise HLS/m3u8 streams hoti hain).
...
// ======================== Done below code mei yeh setting add key hai k aab mei manually select kar skta hu k high quality ya low quality stream chaley , for example internet speed 40kbps or 110kbps ===================================
